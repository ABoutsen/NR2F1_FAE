...
ggplot(data_long[c(2,3,7,16,17,21),], aes(fill=Cluster, y=proportion, x=Cluster, alpha = condition)) +  
        geom_bar(position="dodge", stat="identity", color="black", width = 0.70) +
        scale_alpha_discrete(range= c(1, 0.65)) +
        theme_classic() +
        ylim(0,max_y) +
        ylab("Proportion of cells") +
        scale_fill_manual(values=c(colors_PN[2], colors_PN[1], colors_PN[3]))+
